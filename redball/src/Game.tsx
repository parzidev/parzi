...
const initialProgress = () => {
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        unlocked: Math.max(1, Math.min(LEVEL_COUNT, saved.unlocked || 1)),
        scores: Array.from({ length: LEVEL_COUNT }, (_, index) => saved.scores?.[index] || 0),
      };
    }
  } catch { /* fresh start */ }
  return { unlocked: 1, scores: Array(LEVEL_COUNT).fill(0) };
};
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const release = () => { controls.current.left = false; controls.current.right = false; controls.current.jump = false; };
    window.addEventListener("keydown", down, { passive: false }); window.addEventListener("keyup", up); window.addEventListener("blur", release);
  ...
  useEffect(() => {
    const pauseWhenHidden = () => { if (document.hidden && screen === "game") setPaused(true); };
    ...
    <main className="app-shell">
      ...
      {screen === "levels" && (
        <section className="level-screen">
          <header className="level-header"><button className="round-button" onClick={() => setScreen("menu")} aria-label="Ana menüye dön">←</button><div><p className="eyebrow">5 DÜNYA · 50 BÖLÜM</p><h2>Bölümünü seç</h2></div><div className="total-stars">★ {progress.scores.reduce((a, b) => a + b, 0)} / {LEVEL_COUNT * 3}</div></header>
